...
class ProductCommand implements CommandInterface {
  ...
  public function onCommand($strName, $arrArgs){
    switch($strName){
      case 'addFolderStartElement':
        return $this->addFolderStartProduct($arrArgs);
      case 'editFolderStartElement':
        return $this->editFolderStartProduct($arrArgs);
      default:
        return true;
    }
  }
  ...
  /**
   * addFolderStartProduct
   * @param array $arrArgs
   * @return boolean
   * @author Thomas Schedler <tsh@massiveart.com>
   * @version 1.0
   */
  private function addFolderStartProduct($arrArgs){
    try{
      if(array_key_exists('GenericSetup', $arrArgs) && $arrArgs['GenericSetup'] instanceof GenericSetup){
        ...
        $intTemplateId = $this->core->sysConfig->product_types->overview->default_templateId;
        ...
        $objGenericData->Setup()->setElementTypeId($this->core->sysConfig->product_types->overview->id);
  ...
  /**
   * editFolderStartProduct
   * @param array $arrArgs
   * @return boolean
   * @author Thomas Schedler <tsh@massiveart.com>
   * @version 1.0
   */
  private function editFolderStartProduct($arrArgs){
    try{
      if(array_key_exists('GenericSetup', $arrArgs) && $arrArgs['GenericSetup'] instanceof GenericSetup){
        $objGenericSetup = $arrArgs['GenericSetup'];
        ...
        $this->getModelProducts($arrArgs)->updateFolderStartProduct($intFolderId, $arrProperties, $arrTitle);
        return true;
      }else{
        throw new Exception('There ist now GenericSetup in the args array!');
      }
   ...
   * @version 1.0
   */
  protected function getModelProducts($arrArgs){
    if (null === $this->objModelProducts) {
      ...
      $this->objModelProducts->setLanguageId($arrArgs['LanguageId']);
    }
